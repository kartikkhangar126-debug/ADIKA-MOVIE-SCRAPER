...
function rewriteM3u8(body, url) {
  ...
  return body.split('\n').map(line => {
    ...
    // Handle URI attributes in tags (for Audio tracks/Sub-playlists)
    if (t.startsWith('#')) {
      return line.replace(/URI=["']([^"']+)["']/, (match, p1) => {
        const abs = p1.startsWith('http') ? p1 : p1.startsWith('/') ? origin + p1 : baseDir + p1;
        return `URI="/api?url=${encodeURIComponent(abs)}"`;
      });
    }

    // Handle direct segment links
...
module.exports = async function handler(req, res) {
  ...
  res.setHeader('Content-Type', 'application/json');
  try {
    const data = await getStream(q.id, q.s, q.e);
    ...
    // Return the stream URL (proxied), subtitles and metadata
    res.end(JSON.stringify({
      stream_url: baseUrl + '/api?url=' + encodeURIComponent(playlist),
      subtitles: data?.stream?.captions || [],
      poster: data?.metadata?.poster_path ? `https://image.tmdb.org/t/p/original${data.metadata.poster_path}` : null,
      title: data?.metadata?.title || data?.metadata?.name || 'Unknown'
    }));
  } catch (err) {
    res.statusCode = 500;
